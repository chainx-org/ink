...
impl<E> type_metadata::HasTypeId for XrmlEnv<E>
where
...
#[cfg(feature = "ink-generate-abi")]
impl<E> type_metadata::HasTypeDef for XrmlEnv<E> {
    fn type_def() -> type_metadata::TypeDef {
        type_metadata::TypeDefStruct::new(vec![]).into()
    }
}

impl<T> EnvTypes for XrmlEnv<T>
where
    T: EnvTypes,
{
...
macro_rules! impl_get_property_for {
    ( $( ($name:ident use $sys_fn:expr), )* ) => {
        $(
            impl<T> GetProperty<property::$name<Self>> for XrmlEnv<T>
            where
                T: EnvTypes,
            {
                fn get_property<I>(
...
impl<T> GetProperty<property::Input<Self>> for XrmlEnv<T>
where
    T: EnvTypes,
{
    fn get_property<I>(
...
impl<T> SetProperty<property::RentAllowance<Self>> for XrmlEnv<T>
where
    T: EnvTypes,
{
    #[cfg(not(feature = "old-codec"))]
    ...
    fn set_property<O>(
        ...
        O: old_scale::Output + AsRef<[u8]> + Reset,
    {
        buffer.reset();
        value.encode_to(buffer);
        ext::set_rent_allowance(buffer.as_ref());
        ...
        R: scale::Decode,
    {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "old-codec")] {
                ...
                Decode::decode(&mut &buffer.as_mut()[0..req_len]).ok_or(Error::InvalidStorageRead)
            } else {
