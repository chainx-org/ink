...
mod xrc20 {
    use ink_core::env::DefaultXrmlTypes;
    ...
    impl XRC20 {
        ...
        fn destroy(&mut self, value: u64) -> bool {
            ...
            let convert_to_asset_call = Call::XContracts(chainx_calls::XContracts::<
                DefaultXrmlTypes,
            >::convert_to_asset(
                owner, value
            ));
    ...
    mod tests {
        ...
        fn transfer_works() {
            ...
            // Deploy the contract with some `init_value`
            let mut xrc20 =
                XRC20::deploy_mock(1234, name.clone(), symbol.clone(), decimals);
